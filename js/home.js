window.onload = function () {
  ...
  let ies = contast.querySelectorAll("i")
  let imgs = document.querySelector(".imgs").querySelectorAll("div")  
  for (let i=0;i<ies.length;i++){
    // 鼠标移入的效果
    ies[i].addEventListener('mouseover',()=>{
      imgs[i].style.display='block'
    })
    // 鼠标移出的效果
    ies[i].addEventListener('mouseleave',()=>{
