window.onload = function () {
	...
	if(userWords.value && userName.value){
		...
		btn.onclick = function(){
			...
			newLi.innerHTML = '<img src="../img/09.jpg" alt=""><div class="right"><p class="top"><span class="name">'+userName.value+'</span><a href="javascript:;" class="remove">删除</a></p><p class="words">'+userWords.value+'</p></div>';
			// 将创建好的li添加到ul中
			...
			for(let i=0; i<remBtn.length; i++){
				remBtn[i].onclick = function(){
					let t = 0;
					let myLi = this.parentNode.parentNode.parentNode;
					let start = myLi.offsetHeight;
					let end = 0;
					let change = end - start;
					let maxT = 50;
					let time = setInterval(function(){
	...
	disp.addEventListener('click',none_block)
	// ----------------------------------------------------------------------------------------
	// 音乐播放器的js
	//  获取元素的dom
// 要操作的元素
...
// 专辑名数组
let albums=['喜帖街','反方向的钟','遥远的她','不能说的秘密','枫','搁浅',
'黑色毛衣','蒲公英的约定','，Let me down slowly','Comethru']

// 歌曲信息数组
let track_names=['谢安琪-喜帖街','周杰伦-反方向的钟','张学友-遥远的她','周杰伦-不能说的秘密','周杰伦-枫','周杰伦-搁浅',
'周杰伦-黑色毛衣','周杰伦-蒲公英的约定','Alec-Let Me Down Slowly','Jeremy-Comethru']
...
function selectTrack(flag){
		...
		if(cur_index>-1 && cur_index<albums.length){
				...
				// 设置音频路径
				audio.src='../source/music/0'+(cur_index+1)+cur_track_name+'.mp3';
				if(flag!=0){
						// 当切换上一首,下一首时,自动播放
						audio.play();
						player_track.classList.add('active');
